...
export default function BookingModal({
  ...
  useEffect(() => {
    ...
    const fetchAvailability = async () => {
      try {
        const [appointments, blockedSlotsList, blockedDatesList] =
          await Promise.all([
            getAppointments({ date: selectedDate.fullDateString }),
        ...
        // Active appointments that occupy a slot
        const activeBookedTimes = appointments
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      if (result.success && result.appointment) {
        ...
        const activeBookedTimes = updatedAppointments
          .filter((a) => a.status === "pending" || a.status === "confirmed")
          .map((a) => a.appointment_time);
